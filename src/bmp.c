...
static result_t read_palette(FILE *fp, bmp_header_t *header, image_t *img) {
    ...
    int32_t color_size = (header->info.biSize == CORE_HEADER_SIZE ? 3 : 4);
    int32_t palette_size = header->file.bfOffBits - FILE_HEADER_SIZE
        - header->info.biSize;
    int32_t palette_num = palette_size / color_size;
    int32_t palette_max = (1 << header->info.biBitCount);
    if ((uint32_t)palette_num < header->info.biClrUsed) {
        // 色数よりパレットが小さいので異常
        return FAILURE;
    }
    ...
    if (header->info.biClrUsed != 0 && header->info.biClrUsed < (uint32_t)palette_num) {
    ...
    img->palette_num = (uint16_t)palette_num;
...
static result_t read_bitmap_32(
    ...
    for (y = height - 1; y >= 0; y--) {
        ...
        for (x = 0; x < width; x++) {
            ...
            img->map[y][x].c.r = (uint8_t)CMASK(tmp, header->cmasks[0]);
            img->map[y][x].c.g = (uint8_t)CMASK(tmp, header->cmasks[1]);
            img->map[y][x].c.b = (uint8_t)CMASK(tmp, header->cmasks[2]);
            if (header->cmasks[3].mask == 0) {
                img->map[y][x].c.a = 0xff;
            }
            else {
                img->map[y][x].c.a = (uint8_t)CMASK(tmp, header->cmasks[3]);
            }
        }
    }
    free(buffer);
...
image_t *read_bmp_stream(FILE *fp) {
    ...
    if ((img = allocate_image(header.info.biWidth, height, (uint8_t)color_type)) == NULL) {
        return NULL;
    }
...
static result_t write_header(
    ...
    bs_write16(&bs, (uint16_t)bc);  // biBitCount
 ...
 * @return 成否
 */
#pragma warning(disable: 4100) // bc未使用
static result_t write_bitmap_24(FILE *fp, image_t *img, int bc, int stride) {
    UNREFERENCED_PARAMETER(bc);
    result_t result = FAILURE;
    int32_t x, y;
        ...
        for (x = 0; x < stride; x += count) {
            if (step[x] < 2) {
                ...
                if (reduction > 2) { // 絶対モード
                    *work++ = 0;
                    num = count * cpb;
                    if (num + x * cpb > (int32_t)img->width) {
                        num--;
                    }
                    *work++ = (uint8_t)num;
                    for (i = x; i < x + count; i++) {
                    ...
                    for (i = x; i < x + count; i += step[i]) {
                        num = step[i] * cpb;
                        if (num + i * cpb > (int32_t)img->width) {
                            num--;
                        }
                        *work++ = (uint8_t)num;
                        *work++ = raw[i];
